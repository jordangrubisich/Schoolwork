...
class IP_Header:
    ...
    def build_ip(self, ipv4_header):
        self.get_total_len(ipv4_header[2:4])
        self.get_IP(ipv4_header[12:16],ipv4_header[16:20])
...
class TCP_Header:
    ...
    def build_tcp(self,tcp_header):
        self.get_src_port(tcp_header[:2])
        self.get_dst_port(tcp_header[2:4])
        self.get_seq_num(tcp_header[4:8])
        self.get_ack_num(tcp_header[8:12])
        self.get_flags(tcp_header[13:14])
        self.get_window_size(tcp_header[14:15],tcp_header[15:16])
      
        self.checksum = tcp_header[16:18]
        self.ugp = tcp_header[18:20]
...
class packet():
    ...
    def set_data_len(self):
        self.data_len = self.IP_header.total_len - (self.IP_header.ip_header_len + self.TCP_header.data_offset)
    
    def is_corresponding(self,  p):
        match = False
        this_syn = self.TCP_header.flags["SYN"]
        comp_syn = p.TCP_header.flags["SYN"]
        comp_ack = p.TCP_header.flags["ACK"]
        if comp_ack:
            if this_syn == comp_syn:
                if p.TCP_header.ack_num == self.TCP_header.seq_num +1:
                    match = True
            elif this_syn:
                if p.TCP_header.seq_num == self.TCP_header.ack_num:
                    match = True
            else:
                if p.TCP_header.ack_num == self.TCP_header.seq_num +self.data_len:
                    match = True
        return match
